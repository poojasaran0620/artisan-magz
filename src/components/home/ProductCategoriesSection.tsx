...
export const ProductCategoriesSection: React.FC<ProductCategoriesSectionProps> = ({
  ...
  return (
    <section id="best-selling" className="bg-[#FDFCF5] scroll-mt-20">
      {/* ========================================================================= */}
      {/* PART 1: HIGHLIGHTED "BEST SELLING" FULL-WIDTH SECTION                    */}
      {/* Edge-to-edge pink background, no box container, no borders              */}
      {/* ========================================================================= */}
      <div className="w-full bg-gradient-to-br from-[#9E7864] via-[#B76E79] to-[#804D55] text-white py-12 sm:py-16 px-4 sm:px-8 relative overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto relative z-10 space-y-6">
          ...
          {/* Spotlight Single Bestseller Card: Custom Magazine (Larger size) */}
          <div className="max-w-md sm:max-w-lg mx-auto">
            <motion.div
              whileHover={{ y: -6, scale: 1.02 }}
              transition={springs.smooth}
              className="bg-[#FFFDF9] rounded-3xl p-5 sm:p-7 text-charcoal shadow-luxury border border-white/80 flex flex-col items-center text-center gap-4 relative overflow-hidden group cursor-pointer"
            >
              {/* Bestseller Crown Tag */}
              <div className="absolute top-3.5 right-3.5 z-10 bg-charcoal text-[#FDFCF5] text-[11px] font-bold px-3 py-1 rounded-full shadow-sm flex items-center gap-1">
                <span>#1 Bestseller 🔥</span>
              </div>

              {/* Magazine Visual - Significantly increased size */}
              <div
                onClick={() => onSelectProduct(magazineProduct.id)}
                className="w-60 sm:w-72 md:w-80 shrink-0 cursor-pointer pt-2"
              >
                <div className="relative aspect-[3/4] rounded-2xl overflow-hidden bg-cream-100 shadow-luxury border border-taupe-200/50">
                  <img
              ...
              {/* Title & Downside Build Option (Icon in front of Build removed) */}
              <div className="w-full space-y-3 pt-1">
                <h3
                  onClick={() => onSelectProduct(magazineProduct.id)}
                  className="font-serif text-2xl sm:text-3xl font-bold text-charcoal hover:text-roseGold transition-colors cursor-pointer"
                >
                  Magazines
                </h3>
                ...
                <motion.button
                  ...
                  onClick={() => onSelectProduct(magazineProduct.id)}
                  className="w-full py-3 sm:py-3.5 bg-charcoal hover:bg-charcoal-dark text-[#FDFCF5] rounded-xl text-sm font-bold shadow-soft hover:shadow-soft-lg transition flex items-center justify-center gap-2 cursor-pointer group/btn"
                >
                  <span>Build</span>
                  <ArrowRight className="w-4 h-4 text-roseGold-light group-hover/btn:translate-x-1 transition-transform duration-300" />
                </motion.button>
              </div>
            </motion.div>
          </div>
